...
if($subastarow['YEAR(fecha)']==$fecha['year'] && $subastarow['MONTH(fecha)']==$fecha['mon'] && $subastarow['DAY(fecha)']==$fecha['mday'] && $subastarow['HOUR(fecha)']==$fecha['hours'] && $subastarow['MINUTE(fecha)']==$fecha['minutes']){
    $comenzar=true;
    $comenzar1=mysqli_query($con, "UPDATE Subasta SET actual=true WHERE ID_Subasta=".$_SESSION['subasta']);
}
...
if($subastarow['precio_actual']==$loterow['precio_minimo']){
    $acabada= mysqli_query($con, "UPDATE Subasta VALUE SET actual=false, realizada=true WHERE ID_Subasta=".$subastarow['ID_Subasta']);
